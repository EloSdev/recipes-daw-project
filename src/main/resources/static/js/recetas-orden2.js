...
// Función principal para cargar las recetas, que ahora acepta parámetros de "orden" y "search"
async function cargarRecetas(page = 0, orden = 'fechaDesc', search = '') {
    try {
        // Construir la URL con parámetros de paginación, orden y búsqueda
...
// Función para actualizar la paginación, ahora recibe "orden" y "search"
...
document.getElementById("orden-recetas").addEventListener("change", function(event) {
    ...
    cargarRecetas(0, currentOrder, currentSearch); // Recargar recetas desde la primera página con el nuevo orden
});

// Evento para buscar en tiempo real en el campo de búsqueda
document.querySelector('.search-input').addEventListener('input', function (event) {
    currentSearch = event.target.value;
    cargarRecetas(0, currentOrder, currentSearch); // Recargar recetas desde la primera página con el término de búsqueda
});
