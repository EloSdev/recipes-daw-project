...
async function cargarRecetas(page = 0, orden = "fechaDesc", search = "") {
  try {
    ...
    if (!response.ok) {
      throw new Error(
        `Error al cargar las recetas: ${response.status} ${response.statusText}`
      );
    }
    ...
    if (data && data.content) {
      ...
      document.getElementById("recetas-list").innerHTML =
...
function actualizarPaginacion(totalPages, currentPage, orden, search) {
  ...
  if (currentPage > 0) {
    ...
    prevButton.addEventListener("click", () =>
      cargarRecetas(currentPage - 1, orden, search)
    );
...
async function mostrarRecetas(recetas, recetasVotadas = new Set()) {
  ...
  recetas.forEach((receta) => {
    ...
    const likeButtonClass = recetasVotadas.has(receta.id)
      ? "like-btn liked"
      : "like-btn";
    const likeButtonClassI = recetasVotadas.has(receta.id)
      ? "fa fa-lock"
      : "fas fa-thumbs-up";
  ...
  fetch(`/api/recetas/${recetaId}/like`, {
    ...
    .then((data) => {
      if (data) {
        ...
        likesCountSpan.textContent = data.likes + " votos";
...
async function cargarDetallesReceta(recetaId) {
  try {
    ...
    if (!response.ok) {
      throw new Error(
        `Error al cargar los detalles de la receta: ${response.statusText}`
      );
    }
